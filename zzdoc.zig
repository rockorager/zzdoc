...
const Parser = struct {
    ...
    fn getCh(self: *Parser) ?u8 {
        ...
        if (b == null) {
            b = self.reader.takeByte() catch return null;
        }
    ...
    fn pushStr(self: *Parser, str: []const u8) !void {
        self.str = std.Io.Reader.fixed(str);
    }
    ...
    fn parsePreamble(self: *Parser) !void {
        ...
        while (self.getCh()) |ch| {
            switch (ch) {
                ...
                '-',
                '.',
                => try name.append(self.allocator, @as(u8, @intCast(ch))),
    ...
    fn parseSection(self: *Parser) ![]const u8 {
        ...
        while (self.getCh()) |ch| {
            switch (ch) {
                ...
                'A'...'Z',
                'a'...'z',
                => try section.append(self.allocator, @as(u8, @intCast(ch))),
                ')' => {
                    ...
                    return try section.toOwnedSlice(self.allocator);
                },
    ...
    fn parseExtra(self: *Parser) ![]const u8 {
        ...
        try extra.append(self.allocator, '"');
        while (self.getCh()) |ch| {
            switch (ch) {
                '"' => {
                    try extra.append(self.allocator, '"');
                    return try extra.toOwnedSlice(self.allocator);
                },
                '\n' => return error.UnclosedExtraPreambleField,
                else => {
                    try extra.append(self.allocator, ch);
                },
            }
        }
        return error.UnclosedExtraPreambleField;
        ...
        outer: while (self.getCh()) |ch| {
            switch (ch) {
                ...
                ' ' => {
                    ...
                    if (std.mem.indexOf(u8, buffer.items, "T}")) |_|
                        return error.IllegalCellContents;
                    cur_cell.?.contents = try buffer.toOwnedSlice(allocator);
                    continue :outer;
                },
            ...
            switch (ch_) {
                ' ' => {
                    ...
                    while (self.getCh()) |char_| {
                        switch (char_) {
                            '\n' => break,
                            else => try buffer.append(allocator, char_),
                        }
                    }
                },
                '\n' => continue,
...
test "preamble: expects section to be legit" {
    var fixed = std.Io.Reader.fixed("test(100)\n");
    var parser = try testParserFromSlice(&fixed);
...
test "preamble: expects section to be legit with subsection" {
    var fixed = std.Io.Reader.fixed("test(100hello)\n");
    var parser = try testParserFromSlice(&fixed);
    try std.testing.expectError(error.InvalidSection, parser.parsePreamble());
}

test "preamble: expects section not to contain a space" {
    var fixed = std.Io.Reader.fixed("test(8 hello)\n");
    var parser = try testParserFromSlice(&fixed);
    parser.parsePreamble() catch return;
    try std.testing.expect(false);
}
...
test "preamble: accepts a valid preamble with subsection" {
    var fixed = std.Io.Reader.fixed("test(8hello)\n");
    var parser = try testParserFromSlice(&fixed);
    try parser.parsePreamble();
}
